...
public class FileDecoder
{
    ...
    public FileDecoder(String fileName) {
      ...
      this.fileName = "D:\\Engineering\\CCE\\systems programming\\Systems Parser Final\\Systems Parser Final\\Systems Parser Final\\src\\systems\\parser\\test.txt";
   ...
   public  int getDecimal(String hex)
   {
    ...
    int opcode_generate ()
    {
        ...
        {   
            ...
            else if(listfile.get(i).mnemonic.equalsIgnoreCase("byte"))
            {
                
                String a[]=listfile.get(i).operands.split("'");
                if(a[0].equalsIgnoreCase("C")){
                System.out.println(a[1]);
                System.out.println(ASCIItoHEX(a[1]));
                listfile.get(i).op=ASCIItoHEX(a[1]);
                l.opcode=l.opcode+ASCIItoHEX(a[1]);
                }
              else if(a[0].equalsIgnoreCase("X")){
                  
                     listfile.get(i).op=a[1];
                   l.opcode=l.opcode+a[1];
                }
            }
            else if(listfile.get(i).mnemonic.equalsIgnoreCase("word"))
            {
                
                String b =listfile.get(i).operands;
                    if(b.length()==6)
                     l.opcode=l.opcode+b; 
                   if(b.length()==5)
                     l.opcode=l.opcode+"0"+b;
                    if(b.length()==4)
                     l.opcode=l.opcode+"00"+b;
                 if(b.length()==3)
                     l.opcode=l.opcode+"000"+b;
                 if(b.length()==2)
                     l.opcode=l.opcode+"0000"+b;
                  if(b.length()==1)
                     l.opcode=l.opcode+"00000"+b;
