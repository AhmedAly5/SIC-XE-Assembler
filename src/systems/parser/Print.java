...
public class Print {
    ...
    void print(ArrayList<ListFile> x,ArrayList<lblfile> y,int n,int error) {
        try {
            ...
            writer1 = new PrintWriter("D:\\Engineering\\CCE\\systems programming\\Systems Parser Final\\Systems Parser Final\\Systems Parser Final\\src\\systems\\parser\\Output.txt");
            writer1.print("");
            writer1.close();
            //try (FileWriter writer = new FileWriter("C:\\Users\\Lenovo\\Desktop\\Systems Parser\\src\\systems\\parser\\Output.txt", true)) {
            try (FileWriter writer = new FileWriter("D:\\Engineering\\CCE\\systems programming\\Systems Parser Final\\Systems Parser Final\\Systems Parser Final\\src\\systems\\parser\\Output.txt", true)) {
             //   writer.write("\r\n");
                ...
                for (int i=0; i <x.size(); i++)              
                {
                    if(!x.get(i).commentline)
                    {
                        ...
                        if(x.get(i).label == null)
                            writer.write(x.get(i).line + "\t\t" + toHexa(x.get(i).memory)+" " +x.get(i).op + "\t\t" + "      " + "\t\t" + x.get(i).mnemonic+"\t\t"+x.get(i).operands+"\t\t");
                       else
                            writer.write(x.get(i).line + "\t\t" + toHexa(x.get(i).memory) +" "+ x.get(i).op+"\t\t" + x.get(i).label + "\t\t" + x.get(i).mnemonic+"\t\t"+x.get(i).operands+"\t\t");   
